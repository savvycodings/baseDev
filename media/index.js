...
const belongsTo = (extension) =>
  extensions.find(({ extensions }) =>
    extensions.some((filetype) => extension.includes(filetype)),
  )?.type
...
export const Media = ({ className, media, onLoad, ...props }) => {
  ...
  console.warn(
    'media not supported, please check if extension is correct or should be added to media-switch component',
  )
